define(['knockout', 'vega'], function(ko, vega) {
            ...
            scales: [
                {
                    ...
                    type: 'time',
                    range: 'width',
                    domain: {data: 'timeseries', field: 'data.date'}
                },
                {
                    name: 'y',
                    type: 'linear',
                    range: 'height',
                    nice: true,
                    domain: {data: 'timeseries', field: 'data.value'}
                },
                {
                    name: 'color',
                    type: 'ordinal',
                    ...
                    marks: [
                        {
                            ...
                            properties: {
                                enter: {
                                    strokeWidth: {value: value.strokeWidth}
                                },
                                update: {
                        ...
                        },
                        {
                            ...
                            from: {
                                transform: [
                                    {type: 'filter', test: 'index===data.length-1'}
                                ]
                            },
                            properties: {
                                enter: {
    ...
    ko.bindingHandlers.vegaTime = {
                ...
                if (element.view){
                    ...
                    if (dimensions !== value) {
                        element.view
                            ...
                            .update({duration: value.updateDuration});
                    }
                }
            });
            return {
        ...
        update: function(element, valueAccessor){
            ...
            if (value.data !== null) {
                // NOTE: if datasets is emptied, it's faster for some reason
                ...
                if(element.view && value.data.length) {
                    ...
                    element.view.data(parsed).update({duration: value.updateDuration});
                } else {
